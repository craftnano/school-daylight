...
4. **Use AI to contextualize, not to judge.** Apply AI responsibly to interpret data within real-world constraints — legal mandates, natural disasters, policy changes — because schools operate in contexts that numbers alone cannot capture.
...
**They measure demographics, not quality.** Research from Stanford and UMass demonstrates that school ratings correlate heavily with parent income and neighborhood wealth. Education professor Jack Schneider: they "are ostensibly telling us about the quality of schools when they are really telling us about the privilege of neighborhoods." This drives residential segregation — families chase high numbers into affluent, predominantly white communities, bypassing diverse schools with strong educators and real student growth.
...
We tested this. The answer is: **LLMs cannot do this on the fly.**
...
**The analogy:** Preparing a client for a compliance audit. Here's what the data says. Here's what's normal. Here's what stands out. Here's what we don't know. Here's what to ask next.
...
| **DonorsChoose** | Teacher-initiated funding requests by school — reveals resource gaps and teacher engagement | Public API, school-level | Real-time |
| **US DOE Recognition Programs** | Blue Ribbon, Green Ribbon, NBCT counts | Published lists, searchable | Annual |
| **Local Election Data** | School levy/bond passage rates — community investment signal | County auditor records, state databases | Per election cycle |
...
- Handles caveat language and the "What We Don't Have" disclosure section
...
2. **Academics in context** — Proficiency and growth scores benchmarked against district, state, and demographic peers. Includes the demographic-adjusted performance flag: is this school outperforming, meeting, or underperforming what its poverty rate would predict? This is the single most important insight in the briefing — it separates schools where educators are genuinely strong from schools where high scores simply reflect affluent zip codes.
...
ProPublica also built **"Has Your School Been Investigated for Civil Rights Violations?"** — a lookup tool identifying OCR investigations and allegation categories from federal case-status data (FOIA-obtained resolved cases). This is exactly the reputation/investigations layer in our briefing. It already exists as a concept — we should pull from it rather than recreate it.
...
- **CRDC API** — GSA's federal APIs inventory lists a "Civil Rights Data Collection API" pointing to a usedgov.github.io page. The page returns 404. Suggests deprecation or a non-authoritative listing never maintained. We plan on bulk-file ingestion and our own stable interfaces, not a reliable official API that doesn't exist.
...
The cost structure that killed ProPublica's version no longer exists. The team they needed is a conversation with an LLM. The maintenance burden that made them stop updating — re-ingesting new CRDC cycles, regenerating briefings, updating narratives — is a batch job running overnight for $25.
...
### 1. CRDC Data Is Structurally Late and Noisy
...
### 2. Restraint/Seclusion Data Is a Landmine
...
### 3. Suppression and Small-N Make School-Level Equity Comparisons Unstable
...
**Design response:** Real but not fatal. The briefing already has a "What's Missing" section that discloses suppressions explicitly. District-level data often survives suppression when school-level doesn't — use district as fallback with clear labeling. Show what we can, disclose what we can't, explain why. The briefing's value isn't that every cell is populated — it's that parents know which cells are empty and why.

### 4. Parents Might Not Want Complexity
...
**Design response:** This assumes we need viral growth. We don't. We need a tool useful to the parent who finds it. The product will also serve journalists, advocates, school board members, and policy staff as power users — ProPublica explicitly designed Miseducation for this dual audience. That said, the briefing should offer progressive disclosure: a headline summary at the top (2-3 sentences, what stands out), expandable detail sections below. The CMS hospital comparison model — show methodology, let users drill down, be explicit about update cadence and peer grouping — is the right design pattern.

### 5. Political Sensitivity Around Civil Rights Data
...
2. **CRDC data freshness:** The 2021-22 CRDC reflects a school year still heavily affected by COVID. The 2023-24 data was submitted through April 2025 and was expected to be published by end of 2025. Is it available yet? If not, how prominently do we caveat the COVID-era data?
...
4. **Growth data availability:** Washington publishes student growth percentiles through OSPI, but accessibility varies. Can we reliably get school-level growth scores for the WA launch?
...
8. **Political durability:** If federal CRDC collection or publication is disrupted (as happened with EPA's EJSCREEN), how quickly can we pivot to state-only data sources? What's the minimum viable briefing without CRDC?

9. **ProPublica data reuse:** Can we incorporate or reference ProPublica's OCR investigations lookup data and Miseducation's historical disparity calculations, or do we need to build from primary sources? What licensing applies to their published datasets?

---

*This document describes what we're building and why. The next documents are: data dictionary (mapping source columns to briefing fields), briefing template (structured schema for repeatable generation), and user flow (one parent, one school, one briefing).*
