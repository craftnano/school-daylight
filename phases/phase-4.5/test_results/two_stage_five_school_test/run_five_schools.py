# PURPOSE: Two-stage pipeline hallucination + editorial validation on 5 schools
# INPUTS: Cached raw responses from Round 2 test run (phases/phase-4.5/test_results/raw_responses/)
# OUTPUTS: Per-school JSON files + summary.json + console summary in this directory
# DOES NOT: Modify any existing prompts, configs, MongoDB, or pipeline output
# MODELS: Haiku 4.5 (Stage 1 triage), Sonnet 4.6 (Stage 2 narrative)
# PROMPT FIXES APPLIED: (1) Stage 1 date pass-through, (2) Stage 2 gender-neutral students,
#                       (3) Stage 2 death-location stripping
# RUNS: Sonnet 4.6 first run with the three Stage-2 fixes from the April 2026 resume doc.
# This script supersedes run_four_schools.py by adding Juanita HS as the 5th school.
...
SCHOOLS = [
    ...
    },
    {
        ...
        "label": "Juanita HS",
        "reason": "Original single-stage hallucination case (fabricated 'placed on leave but later cleared'). Severity-exception keeper.",
    },
]

# --- Stage 1 Prompt: Haiku Triage ---
# Recency rules from sonnet_layer3_prompt_test_v2.txt; date pass-through fix applied April 2026.
...
# Includes Rule 4 strengthening (death-location/circumstance) and Rule 9 (gender neutralization for students).
...
def check_death_circumstances(narrative):
    """
    Check for death-location, manner-of-death, and physical-circumstance leakage
    in the narrative. These should be stripped under Stage 2 Rule 4.
    ...
    for phrase in forbidden_phrases:
        if phrase in narr_lower:
            violations.append(f"death-circumstance phrase: '{phrase}'")
    return violations


def check_gendered_pronouns_for_students(narrative):
    """
    Heuristic check for gendered pronouns appearing near 'student' references.
    Flags potential Rule 9 violations for human review.
    """
...
def run_school(api_client, school_info, timestamp):
    ...
    # Sanity check: did Haiku include the date field? (Fix #1)
    ...
    for item in included:
        ...
        date = item.get("date", "MISSING")
        rationale = item.get("rationale", "")
        print(f"    INCLUDE [{idx}] ({theme}, {date}): {rationale[:80]}")
...
def main():
    ...
    print(f"Two-Stage 5-School Validation — {timestamp}")
    ...
    total_student_ids = 0
    total_death_violations = 0
    total_gender_flags = 0
    total_haiku_in = 0
    total_haiku_out = 0
    total_sonnet_in = 0
    total_sonnet_out = 0
    ...
    for nid, r in all_results.items():
        ...
        total_death_violations += len(r.get("death_circumstance_violations", []))
        total_gender_flags += len(r.get("gendered_student_flags", []))
    ...
    print(f"Total death-circumstance violations: {total_death_violations}")
    print(f"Total gendered-student flags: {total_gender_flags}")
    ...
    summary = {
        ...
        "models": {"stage1_haiku": HAIKU_MODEL, "stage2_sonnet": SONNET_MODEL},
        "totals": {
            "hallucinations": total_hallucinations,
            "student_id_leaks": total_student_ids,
            "death_circumstance_violations": total_death_violations,
            "gendered_student_flags": total_gender_flags,
        },
        "cost": {
    ...
    for nid, r in all_results.items():
        if "error" in r:
            ...
            summary["per_school"][nid] = {
                ...
                "included_with_date": r["stage1"].get("included_with_date", 0),
                ...
                "death_circumstance_violations": len(r.get("death_circumstance_violations", [])),
                "death_circumstance_details": r.get("death_circumstance_violations", []),
                "gendered_student_flags": len(r.get("gendered_student_flags", [])),
                "gendered_student_details": r.get("gendered_student_flags", []),
            }
