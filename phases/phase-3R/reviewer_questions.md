...
3. **Additional Nebraska variables.** Of the variables in Nebraska's full 27-variable set that were not initially in the v1 commit, teacher experience and the finance dimension (substituted as district-level average teacher salary) have now been added. Teacher master's degree percentage is excluded on data-availability grounds (no school-level credential data is queryable on data.wa.gov; v2 path identified parsing S-275 raw personnel extracts). Further refinement could consider expanding the finance dimension beyond a single salary variable, or adding community-level variables not in Nebraska's set.
...
To test this commitment empirically, the methodology computation produced parallel peer cohorts using a Nebraska-style variable set (achievement included). Mean Jaccard overlap with the v1 cohorts was 0.385 across the eligible-set intersection (n = 1,170): with K=20 in each cohort, this corresponds to approximately 11 schools shared on average and approximately 9 schools that change. Including achievement in similarity therefore reassigns roughly 45 percent of each cohort. The cohort-level finding establishes that the variable choice is not inconsequential. The flag-level effect (which schools flip flags, magnitude of attenuation) will be reported once Section 2 is locked and the academic flag is computed. The sensitivity analysis is presented as a test of the v1 commit, not as a co-equal candidate methodology.
...
**Q2d. Distance metric.** *Retired and superseded by Q6 (residual correlations and the v2 escalation framing). Q6 develops the distance-metric question with the empirical correlation structure observed in the methodology computation; reviewers should engage Q6 in place of Q2d.*
...
**Q2f. Similarity threshold for the named-peers display.** The briefing surfaces 3 named peer schools per school. For schools whose 3rd-nearest peer falls beyond a similarity threshold (i.e., schools that are structurally unusual), the named-peers display falls back to a notice that no closely-matched peers exist. The threshold is currently TBD; it depends on the empirical distribution of distances in the nearest-neighbor matrix from the methodology computation. What's an appropriate way to set this threshold? Options include: a hard cutoff on raw Euclidean distance, a percentile-based threshold (e.g., schools whose 3rd-nearest peer is in the worst 10% of nearest-neighbor distances), or a relative threshold tied to the within-cohort variance.
...
## Q6. Residual correlations and the v2 escalation framing

**Topic:** Whether the consolidation decision applied during the methodology computation was the right response to the observed correlation structure, whether the residual correlations remaining after consolidation are acceptable as documented limitations, and whether the v2 Mahalanobis escalation framing is methodologically tight.

**Context:** The original Phase 3R commit specified a decision rule. If any pairwise correlation in the standardized variable set exceeded 0.70, two remediation paths would be considered: consolidation of redundant variables, or a switch to Mahalanobis distance. The methodology computation surfaced four clusters of correlated variables that exceeded the threshold (full empirical detail in `phases/phase-3R/experiment/methodology_inspection.md`):
...
- *Cluster 2 (population):* total_population and population_density correlated at r = +0.85 to +0.91 across levels.
- *Cluster 3 (race and English-language status):* race_pct_non_white and ell correlated at r = +0.78 to +0.80.
- *Cluster 4 (FRL with income, Middle level only):* frl correlated with median_household_income at r = -0.76, with per_capita_income at r = -0.74, with bachelors_pct_25plus at r = -0.72.

The v1 commit applied consolidation to Clusters 1 and 2 (Path C): per_capita_income, bachelors_pct_25plus, and total_population are dropped from the variable set. Clusters 3 and 4 are retained as documented residuals on the grounds that dropping race_pct_non_white or ell would lose substantive content (Cluster 3 reflects WA-specific Hispanic agricultural-worker and refugee resettlement geography rather than methodological double-counting; Cluster 4 is partly small-n at the Middle level and partly genuine catchment-district overlap).

**v1 commit:** Path C consolidation. Mahalanobis distance is documented as a v2 escalation path under three explicit trigger conditions rather than as a parallel alternative to consolidation:
...
**Default for v1 without reviewer input:** Consolidated 17-variable set with Euclidean distance retained. Section 1.5.1 of the brief documents the consolidation reasoning; Section 1.5.2 documents the residual correlations and the v2 escalation triggers.

**What we'd want from the reviewer:** Whether consolidation was the right call at the magnitudes observed, or whether the dropped variables (per_capita_income, bachelors_pct_25plus, total_population) carry methodological weight that consolidation discards in a way the reviewer would push back on. Whether the residual correlations (race_pct_non_white ~ ell ~+0.78; frl ~ median_household_income ~-0.76 Middle-only) are acceptable as documented WA-specific limitations or warrant further treatment. Whether the three Mahalanobis v2 trigger conditions are methodologically tight or would benefit from refinement. Whether the redundancy audit should report higher-order structure (principal components, variance inflation factors) in addition to pairwise correlations going forward.

**Stakes:** Moderate to high. The distance metric and variable set jointly determine every cohort assignment. Path C consolidation handles the cleanest cases (the three-way income/education redundancy at r = 0.93; the tautological population/density relationship at WA scale) better than Mahalanobis would, while preserving variable transparency. If the reviewer disagrees with this trade-off, the v2 escalation triggers give a defined reversion path; without that framing, the consolidation choice could be perceived as foreclosing the more sophisticated treatment.

**Related questions:** Q6 is closely related to Q2a (variable selection). Q2d (the original distance-metric question) is retired and superseded by Q6.
