...
To test this commitment empirically, the methodology computation produced parallel peer cohorts using a Nebraska-style variable set (achievement included). Mean Jaccard overlap with the v1 cohorts was 0.399 across the eligible set: with K=20 in each cohort, this corresponds to approximately 11 schools shared on average and approximately 9 schools that change. Including achievement in similarity therefore reassigns roughly 45 percent of each cohort. The cohort-level finding establishes that the variable choice is not inconsequential. The flag-level effect (which schools flip flags, magnitude of attenuation) will be reported once Section 2 is locked and the academic flag is computed. The sensitivity analysis is presented as a test of the v1 commit, not as a co-equal candidate methodology.
