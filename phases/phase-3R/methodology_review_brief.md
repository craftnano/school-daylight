...
### 1.3 Achievement variables: excluded (deviation from Nebraska)

Nebraska and Texas disagree on whether achievement variables belong in the similarity calculation, and the WA methodology had to settle the question one way or the other. Nebraska includes ELA, Math, and Science proficiency in similarity. Texas excludes them. The v1 commit sides with Texas on this single dimension while keeping every other Section 1 framework choice (Euclidean distance, per-level segmentation, K-based cohorts, no geographic constraint) from Nebraska. The deviation is narrow and deliberate, and the rest of this section explains why.

Three concerns support the deviation, and they reinforce each other. The first is mechanical: matching schools partly by achievement creates circularity that systematically attenuates the academic flag toward zero. Schools that underperform their structural peers get matched partly to other underperformers; schools that outperform get matched partly to other outperformers; the comparison is compressed in both directions. The second is communicative: the briefing's named-peers display shows matching variables alongside the academic comparison, and the comparison becomes incoherent to a parent reading "your school is matched to schools that perform similarly" next to "and here is how your school's performance compares to those peers." The third is purposive: peer comparison does substantive work only when the cohort acts as a control group for structural conditions a school does not choose. Including achievement collapses that control group into the outcome variable; excluding it lets the comparison reveal a real differential rather than residual variance after a partial match.

Theoretical reasoning about circularity is not sufficient on its own. The deviation needed an empirical test: in the actual WA data, do achievement-similar peers and structurally-similar peers turn out to be roughly the same schools? If they did, Nebraska's inclusion of achievement would be cosmetic and the deviation moot. The methodology computation tested this directly. It produced parallel peer cohorts using a Nebraska-style variable set with achievement included alongside the v1 cohorts, then measured how much the two assignments overlap using the Jaccard index (the share of schools two cohort assignments have in common: |A ∩ B| divided by |A ∪ B|, where 1.0 means the cohorts are identical and 0 means they share no schools). Mean Jaccard overlap was 0.385 across the eligible-set intersection (n = 1,170), which with K=20 in each cohort corresponds to approximately 11 schools shared on average and approximately 9 schools reassigned. Including achievement in similarity therefore moves roughly 45 percent of each cohort to a different school.

That 45 percent reassignment is the empirical content of the circularity argument; it puts a concrete magnitude on what would otherwise be a theoretical concern. Nine of every twenty peers in a school's cohort change identity depending on whether achievement is included in the similarity calculation, and those identity changes propagate forward into different academic flag assignments downstream. The flag-level effect (which specific schools flip flags, and the magnitude of the attenuation toward zero) will be reported once Section 2 is locked and the academic flag is computed. For Section 1, the cohort-level evidence is sufficient to anchor the deviation. The exclusion of achievement from similarity protects the academic flag, which is the headline output of School Daylight's peer-comparison methodology, from systematic attenuation. The sensitivity analysis is the test that justifies the deviation, not a parallel methodology under evaluation.
...
The empirical magnitude of this residual is documented up to a ceiling of r = 0.81. At per-level sample sizes in the hundreds, the sampling-noise floor on a correlation estimate near 0.80 is approximately ±0.04; treating the difference between 0.80 and 0.81 as substantively meaningful would over-interpret decimal precision. The 0.81 ceiling provides a small buffer for cross-eligible-set sampling variation while remaining tight enough that a structural strengthening of the residual (e.g., r climbing to 0.85) would surface as a methodological concern requiring re-litigation.

Second, FRL correlates with median_household_income at r = -0.76 at the Middle school level only (n = 374). The Middle-level estimate is partly a smaller-n artifact and partly a genuine overlap between school-catchment FRL and district-level economic geography at the level group where catchment-district correspondence is tightest. The residual is accepted as a documented WA-specific limitation rather than driving further consolidation.
